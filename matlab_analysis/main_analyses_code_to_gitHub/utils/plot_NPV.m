%{
    ...
    For each binarizing threshold of AI score, computes and displays NPV 
    and portion of impacted patients
%}


function plot_NPV(label_RS, AI_score, th)

%% compute TNR and impacted patinets at th

NPV_at_th = perfcurve(label_RS, AI_score, 1, ...
                         'XCrit','npv','TVals', th, 'NBoot',1000);

fprintf('NPV (th=%d) = %.3f (%.3f–%.3f)\n', th,NPV_at_th);
...
useBoot = true;
NPV_Y_vec = Compute_NPV_vs_AI(label_RS, AI_score, useBoot);

NPV_X_vec = sort(AI_score);
...
%% display
display_NPV(NPV_X_vec, NPV_Y_vec, Patients_X_vec, patients_VS_AI, th, NPV_at_th(1), impacted_patients_at_th)